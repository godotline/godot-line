...
const TriggerTypeMap: Script = preload("res://addons/dancing_line_importer/scripts/trigger_type_map.gd")

# 仅以下两个组件由专用构建函数（_buildVisibilityTrigger / _buildEnvironmentTrigger / _buildFogTrigger）使用，留在本地
...
# Move/Rotate/Scale 变换触发器（type 5/6/7）的目标 Animator 组件，按 animatorClass 区分挂载。
# 注意：旧的统一 LocalAnimator.gd 已删除；三者分别由 LocalPosAnimator/LocalRotAnimator/LocalScaleAnimator 承担。
...
# 触发器类型映射（TRIGGER_CONFIG / TRIGGER_FIELD_MAP / UNMAPPED_TRIGGER_TYPES）
# 已抽取到独立数据源：addons/dancing_line_importer/scripts/trigger_type_map.gd
# 运行时通过常量 TriggerTypeMap 访问（含 is_mapped / get_config / get_field_rules / record_unmapped 等辅助）。
...
# 变换触发器（type 5/6/7）的待挂载队列：键=EventTrigger 组件节点，值=解析后的动画参数字典。
# 由 _build*Trigger 在创建触发器时填充，在 _createObjects 末尾的 _link_trigger_animators 中按 targetId 挂载到目标节点。
var _pendingAnimatorTriggers: Dictionary = {}


func buildScene(data: Dictionary, levelDataResource: LevelData = null) -> Node3D:
	loadedMeshes.clear()
	loadedTextures.clear()
	_pendingAnimatorTriggers.clear()
	...
	# 汇总本次导入中遇到的未映射触发器类型
	var unmapped: Dictionary = TriggerTypeMap.take_unmapped_summary()
	if not unmapped.is_empty():
		var parts: PackedStringArray = []
		for typeKey: Variant in unmapped.keys():
			parts.append("%d(%s)" % [typeKey, unmapped[typeKey]])
		push_warning("DancingLineImporter: 本次导入发现 %d 个未映射触发器类型: %s（见 trigger_type_map.gd -> UNMAPPED_TRIGGER_TYPES）" % [unmapped.size(), "、".join(parts)])
...
func _createObjects(parent: Node, data: Dictionary) -> void:
	...
	# genius idea: Move/Rotate/Scale 类触发器（EventTrigger + targetId）在目标节点挂载 LocalAnimator 并链接 triggered -> play
	_link_trigger_animators(nodeMap)


# genius idea: 把 EventTrigger(发事件) 与 目标节点上的 Local*Animator(执行动画) 串起来。
# 动画参数在 _build*Trigger 中按段解析后存入 _pendingAnimatorTriggers（键=EventTrigger 组件节点），
# 此处按 targetId 从 nodeMap 取目标节点，挂载对应 Local*Animator 并链接 triggered -> Trigger()。
func _link_trigger_animators(nodeMap: Dictionary) -> void:
	for etVar: Variant in _pendingAnimatorTriggers.keys():
		var et: EventTrigger = etVar as EventTrigger
		if not et:
			continue
		var entry: Dictionary = _pendingAnimatorTriggers[et] as Dictionary
		if entry.is_empty():
			continue
		var targetId: int = entry.get("targetId", -1)
		if not nodeMap.has(targetId):
			push_warning("DancingLineImporter: TransformTrigger targetId=%d 未找到目标节点，无法挂载 Local*Animator" % targetId)
			continue
		var target: Node = nodeMap[targetId]
		if not (target is Node3D):
			push_warning("DancingLineImporter: TransformTrigger targetId=%d 目标不是 Node3D，无法挂载 Local*Animator" % targetId)
			continue
		var animatorClass: Script = entry.get("animator_class", null)
		if animatorClass == null:
			continue
		var animator: AnimatorBase = animatorClass.new() as AnimatorBase
		if animator == null:
			continue
		animator.endOffset = entry.get("endOffset", Vector3.ZERO)
		animator.transformType = entry.get("transformType", AnimatorBase.TransformType.New)
		animator.duration = entry.get("duration", 1.0)
		var easeDict: Dictionary = entry.get("ease", {})
		animator.TransitionType = easeDict.get("trans", Tween.TRANS_LINEAR)
		animator.EaseType = easeDict.get("ease", Tween.EASE_IN_OUT)
		animator.reverse = entry.get("reverse", false)
		target.add_child(animator)
		if not et.triggered.is_connected(animator.Trigger):
			et.triggered.connect(animator.Trigger)
	_pendingAnimatorTriggers.clear()
...
func _createTriggerObject(objData: Dictionary) -> Node:
	# 复用 Trigger.tscn 预制体（Area3D + BaseTrigger + 子 CollisionShape3D），不再手动创建 Area3D
	var triggerScene: PackedScene = load(TRIGGER_TEMPLATE) as PackedScene
	var triggerRoot: Area3D = triggerScene.instantiate() as Area3D
	...
	# 缩放作用于 Area3D 根节点（子 CollisionShape3D 随根节点等比缩放），而非单独缩放碰撞体
	var scale: Vector3 = getVector3FromDict(objData, "scale", Vector3.ONE)
	...
	var config: Variant = TriggerTypeMap.get_config(triggerType)
	if config == null:
		# 未映射 / 占位类型：登记并告警，但仍生成空 Area3D 根节点
		var placeholderName: String = str(objData.get("name", "Trigger"))
		TriggerTypeMap.record_unmapped(triggerType, placeholderName)
		push_warning("DancingLineImporter: 未映射的触发器类型 triggerType=%d (name=%s)，已生成空触发器，详见 trigger_type_map.gd 的 UNMAPPED_TRIGGER_TYPES" % [triggerType, placeholderName])
		triggerRoot.name = "%s_%d" % [placeholderName, objId]
		return triggerRoot

	var cfg: Dictionary = config as Dictionary
	triggerRoot.name = "%s_%d" % [cfg["name"], objId]

	var comp: Node = (cfg["component"] as Script).new()
	comp.name = cfg["componentName"]
	if TriggerTypeMap.TRIGGER_FIELD_MAP.has(triggerType):
		_applyTriggerFields(dataStr, comp, TriggerTypeMap.TRIGGER_FIELD_MAP[triggerType])
...
# 通用字段映射应用器：按 TRIGGER_FIELD_MAP 中的规则把 dataStr 写入组件属性
func _applyTriggerFields(dataStr: String, comp: Node, rules: Array) -> void:
	var parts: PackedStringArray = dataStr.split("|")
	for ruleVar: Variant in rules:
		if not ruleVar is Dictionary:
			continue
		var rule: Dictionary = ruleVar as Dictionary
		var kind: String = rule.get("kind", "float")

		if kind == "special":
			Callable(self, rule["special"]).call(dataStr, comp)
			continue
		if kind == "const":
			comp.set(rule["prop"], rule.get("value", 0))
			continue

		if not rule.has("seg"):
			continue
		var seg: int = rule["seg"]
		if seg >= parts.size():
			continue
		var raw: String = parts[seg].strip_edges()

		match kind:
			"vec3_raw", "vec3_rad":
				var subDelim: String = rule.get("sub", "")
				if subDelim != "":
					var sub: PackedStringArray = parts[seg].split(subDelim)
					if sub.size() >= rule.get("n", 3):
						var x: float = float(sub[0].strip_edges())
						var y: float = float(sub[1].strip_edges())
						var z: float = float(sub[2].strip_edges())
						if kind == "vec3_rad":
							comp.set(rule["prop"], Vector3(deg_to_rad(x), deg_to_rad(-y), deg_to_rad(z)))
						else:
							comp.set(rule["prop"], Vector3(x, y, z))
			"float":
				if raw.is_valid_float():
					comp.set(rule["prop"], float(raw))
			"int":
				if raw.is_valid_int():
					comp.set(rule["prop"], int(raw))
			"fov":
				comp.set(rule["prop"], float(raw) if raw.is_valid_float() else rule.get("default", 80.0))
			"bool":
				comp.set(rule["prop"], raw.to_lower() == "true")
			"ease":
				comp.set(rule["prop"], _parseEaseType(raw))
			"speed":
				var v: float = float(raw) if raw.is_valid_float() else 12.0
				if v > 0.0:
					comp.set(rule["prop"], v)
			"jump_power":
				var sub: PackedStringArray = parts[seg].split(",")
				if sub.size() >= 2:
					var py: float = float(sub[1].strip_edges())
					comp.set(rule["prop"], py if py > 0 else 500.0)
				else:
					comp.set(rule["prop"], float(raw) if raw.is_valid_float() else 500.0)
			"duration_cam":
				var dur: float = 2.0
				if parts.size() >= 10 and parts[9].is_valid_float() and float(parts[9]) > 0.0:
					...
					dur = clamp(5000.0 / sf, 0.1, 10.0) if sf > 10.0 else 2.0
				comp.set(rule["prop"], dur)


# VisibilityTrigger (type 18) — 动态激活/隐藏区域
# 数据格式: "Gone|9372|False|" -> Mode|TargetObjId|DontRevive|
func _buildVisibilityTrigger(dataStr: String, comp: Node) -> void:
	var parts: PackedStringArray = dataStr.split("|")
	if parts.size() < 2:
		return
	...
	comp.set("actives", [singleActive])


# EnvironmentTrigger (type 19) — 环境雾/颜色切换
# 数据格式: "True|Color|True|0.125, 0, 0, 1|True|True|1, 1, 1, 1|2.5|linear"
func _buildEnvironmentTrigger(dataStr: String, comp: Node) -> void:
	var parts: PackedStringArray = dataStr.split("|")
	...
	if parts.size() >= 8 and parts[7].is_valid_float():
		comp.set("duration", float(parts[7]))
	comp.set("fog", fogSetting)


# FogTrigger (type 22)
# 数据格式: "0.01|0.1254902, 0, 0, 1|True|2.5|linear"
func _buildFogTrigger(dataStr: String, comp: Node) -> void:
	var parts: PackedStringArray = dataStr.split("|")
	var fogSetting: FogSettings = FogSettingsClass.new()
	fogSetting.useFog = true
	...
	if parts.size() >= 4 and parts[3].is_valid_float():
		comp.set("duration", float(parts[3]))
	comp.set("fog", fogSetting)


# Transform 触发器（Move=type5 / Rotate=type6 / Scale=type7）共用构建器
# genius idea: 不直接变换节点，只把解析结果存入 _pendingAnimatorTriggers（键=EventTrigger 组件节点），
# 真正的变换由导入后处理器 _link_trigger_animators 在目标节点(按 targetId)挂载 Local*Animator 并链接 triggered -> Trigger()。
# 不在 EventTrigger 上挂任何属性（见架构说明）。
# 数据格式: "dx, dy, dz|ease|targetId|useOffset|flag||tweenTime|reverse"
#   [0]endOffset(目标值/增量) [1]ease(见 TriggerTypeMap.ANIMATOR_EASE_MAP) [2]targetId(目标节点 id)
#   [3]useOffset(true=TransformType.Add / false=New) [4]占位(含义待确认) [5]空(两个|之间)
#   [6]tween 时长 [7]reverse(调换 start/end)
func _buildTransformTrigger(dataStr: String, comp: Node, animatorClass: Script) -> void:
	var et: EventTrigger = comp as EventTrigger
	if not et:
		return
	var parts: PackedStringArray = dataStr.split("|")
	...
	if parts.size() >= 1:
		var sub: PackedStringArray = parts[0].split(",")
		if sub.size() >= 3:
			endOffset = Vector3(float(sub[0].strip_edges()), float(sub[1].strip_edges()), float(sub[2].strip_edges()))

	var easeDict: Dictionary = TriggerTypeMap.get_animator_ease("linear")
	if parts.size() >= 2:
		easeDict = TriggerTypeMap.get_animator_ease(parts[1].strip_edges())

	var targetId: int = -1
	if parts.size() >= 3 and parts[2].strip_edges().is_valid_int():
		targetId = int(parts[2])

	var transformType: AnimatorBase.TransformType = AnimatorBase.TransformType.New
	if parts.size() >= 4 and parts[3].strip_edges().to_lower() == "true":
		transformType = AnimatorBase.TransformType.Add

	var duration: float = 1.0
	if parts.size() >= 7 and parts[6].strip_edges().is_valid_float():
	...
	# [4] 占位（含义待确认）、[5] 空段，跳过。
	_pendingAnimatorTriggers[et] = {
		"animator_class": animatorClass,
		"endOffset": endOffset,
		"ease": easeDict,
		"targetId": targetId,
...
func _buildMoveTrigger(dataStr: String, comp: Node) -> void:
	_buildTransformTrigger(dataStr, comp, LocalPosAnimatorClass)

func _buildRotateTrigger(dataStr: String, comp: Node) -> void:
	_buildTransformTrigger(dataStr, comp, LocalRotAnimatorClass)

func _buildScaleTrigger(dataStr: String, comp: Node) -> void:
	_buildTransformTrigger(dataStr, comp, LocalScaleAnimatorClass)
