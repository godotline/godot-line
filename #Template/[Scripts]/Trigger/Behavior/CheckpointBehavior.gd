...
## CheckpointBehavior - 检查点行为组件
## 当父 BaseTrigger 被触发时记录完整游戏状态，支持 revive 恢复
...
func revive() -> void:
	...
	if main_line.get_tree():
		main_line.get_tree().call_group("death_particles", "queue_free")

	# Kill all tweens
	for tween in main_line.get_tree().get_processed_tweens():
		tween.kill()
	...
	# Restore music to checkpoint position
	...
	# Restore animation to checkpoint position
