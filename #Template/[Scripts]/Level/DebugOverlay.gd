...
func _drawLayout() -> void:
	...
	ImGui.SetNextWindowPos(Vector2(35.0, 35.0), 4) # 4 = CondFirstUseEver，允许用户拖动后记忆位置
	...
	if expanded:
		...
		ImGui.Text("")
		if ImGui.SmallButton("Reload (R)"):
			p.reload()
		# 与键盘 K 同一状态门控：仅 Playing 可判死，防止死亡后反复鞭尸
		if ImGui.SmallButton("Kill (K)") and LevelManager.GameState == LevelManager.GameStatus.Playing:
